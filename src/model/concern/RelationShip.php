...
trait RelationShip
{
     ...
     * @return void
     */
    public function bindRelationAttr($model, array $bind = [])
    {
     ...
     * @return Query
     */
    public static function hasWhere($relation, $where = [], string $fields = '*', string $joinType = '', ?Query $query = null): Query
    {
     ...
     * @return Query
     */
    public static function hasWhereOr($relation, $where = [], string $fields = '*', string $joinType = '', ?Query $query = null): Query
    {
        if (is_array($relation)) {
            [$relation, $alias] = $relation;
        }
   ...
   /**
    ...
    public function bindAttr(string $relation, array $attrs = [])
    {
        ...
        foreach ($attrs as $key => $attr) {
            ...
            if ($attr instanceof Closure) {
                ...
                $value = ($nullsafeVariable1 = $relation) ? $nullsafeVariable1->get($attr) : null;
            }
     ...
     * @return array
     */
    protected function parseMorph($morph): array
    {
        if (is_array($morph)) {
