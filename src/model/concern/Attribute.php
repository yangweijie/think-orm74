...
trait Attribute
{
     ...
     * @return void
     */
    private function initializeData($data, bool $fromSave = false)
    {
        // 分析数据
     ...
     * @return $this
     */
    public function withFieldAttr($attr, ?callable $callback = null)
    {
        if (is_array($attr)) {
            ...
            // 自动追加输出
            $this->appendWeakData('append', $name);
        }
    ...
    protected function readTransform($value, $type)
    {
        ...
        switch ($type) {
            ...
            case 'object':
                return empty($value) ? new \stdClass() : (is_string($value) ? json_decode($value) : json_decode(json_encode($value, JSON_FORCE_OBJECT)));
            case 'json':
                return $typeTransform(Json::class, $value, $this, $param);
            case 'date':
                return $typeTransform(Date::class, $value, $this, $param);
            case 'datetime':
                return $typeTransform(DateTime::class, $value, $this, $param);
            case 'timestamp':
                return $typeTransform(DateTime::class, $value, $this, $param);
            default:
                return $typeTransform($type, $value, $this, $param);
        }
    }
    ...
    protected function writeTransform($value, $type)
    {
        ...
        switch ($type) {
            ...
            case 'json':
                return $typeTransform(Json::class, $value, $this);
            case 'date':
                return $typeTransform(Date::class, $value, $this);
            case 'datetime':
                return $typeTransform(DateTime::class, $value, $this);
            case 'timestamp':
                return $typeTransform(DateTime::class, $value, $this);
            default:
                return $typeTransform($type, $value, $this);
        }
    }
    ...
    {
        $data = $this->getWeakData('data', $this->getMappingName($name), null);
        return $this->hasGetAttr($name) || $data !== null;
    }
     ...
     */
    public function withEnumRead($method = true)
    {
