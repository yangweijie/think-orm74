...
abstract class Model implements JsonSerializable, ArrayAccess, Arrayable, Jsonable, Modelable
{
     ...
     */
    private static function initWeakMap(): void
    {
    ...
    public function __construct($data = [])
    {
        ...
        self::setObjectData($this, $modelData);

        // 设置额外参数
        $this->setOptions(array_diff_key($options, $modelData));
    ...
    {
        $data = self::getObjectData($this);
        return $data[$key][$name] ?? $default;
    }

    /**
     * 追加数据到 WeakMap 的数组中
     */
    private function appendWeakData(string $key, $value): void
    {
        $data = self::getObjectData($this);
        if ($data !== null) {
            if (!isset($data[$key])) {
     ...
     * @return Model|Entity
     */
    public function newInstance($data = [], array $options = [])
    {
    ...
    protected function parseValidate(): string
    {
        ...
        $validate = $auto && strpos(static::class, '\\model\\') !== false ? str_replace('\\model\\', '\\validate\\', static::class) : '';
     ...
     * @return Modelable
     */
    public static function create($data, array $allowField = [], bool $replace = false, string $suffix = ''): Modelable
    {
        $model = new static();
        if (!empty($suffix)) {
     ...
     * @return Modelable
     */
    public static function update($data, $where = [], array $allowField = [], string $suffix = '', bool $refresh = false): Modelable
    {
        $model  = new static();
        if (!empty($suffix)) {
            $model->setSuffix($suffix);
     ...
     * @return $this
     */
    public function inc(string $field, $step = 1, int $lazyTime = 0)
    {
     ...
     * @return $this
     */
    public function dec(string $field, $step = 1, int $lazyTime = 0)
    {
        return $this->set($field, new Express('-', $step, $lazyTime));
    }
