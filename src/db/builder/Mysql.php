...
class Mysql extends Builder
{
    ...
    public function update(Query $query): string
    {
        ...
        $set = [];
        foreach ($data as $key => $val) {
            $set[] = (strpos($key, '->') !== false ? strstr($key, '->', true) : $key) . ' = ' . $val;
        }
     ...
     *
     * @param Query $query  查询对象
     * @param string|int|\think\db\Raw $key 字段名
    ...
    public function parseKey(Query $query, $key, bool $strict = false): string
    {
        ...
        if (strpos($key, '->>') !== false && strpos($key, '(') === false) {
            ...
            return $this->parseKey($query, $field, true) . '->>\'$' . (strncmp($name, '[', strlen('[')) === 0 ? '' : '.') . str_replace('->>', '.', $name) . '\'';
        }
        ...
        if (strpos($key, '->') !== false && strpos($key, '(') === false) {
            ...
            return 'json_unquote(json_extract(' . $this->parseKey($query, $field, true) . ', \'$' . (strncmp($name, '[', strlen('[')) === 0 ? '' : '.') . str_replace('->', '.', $name) . '\'))';
        }
    ...
    {
        if (strncmp($key, "json_unquote(json_extract", strlen("json_unquote(json_extract")) === 0) {
            if ('NULL' === $exp) {
