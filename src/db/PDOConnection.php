...
abstract class PDOConnection extends Connection
{
    ...
    protected function getFieldType(string $type): string
    {
        ...
        switch (true) {
            case strncmp($type, 'set', strlen('set')) === 0:
                return 'set';
            case strncmp($type, 'enum', strlen('enum')) === 0:
                return 'enum';
            case strncmp($type, 'bigint', strlen('bigint')) === 0:
                return 'bigint';
            case strpos($type, 'float') !== false || strpos($type, 'double') !== false || 
            strpos($type, 'real') !== false:
                return 'float';
            case strpos($type, 'int') !== false || strpos($type, 'serial') !== false ||
            ...
            case strpos($type, 'json') !== false:
                return 'json';
            case strncmp($type, 'timestamp', strlen('timestamp')) === 0:
                return 'timestamp';
            case strncmp($type, 'datetime', strlen('datetime')) === 0:
                return 'datetime';
            case strncmp($type, 'date', strlen('date')) === 0:
                return 'date';
            default:
                return 'string';
        }
    ...
    {
        $schema = strpos($tableName, '.') !== false ? $tableName : $this->getConfig('database') . '.' . $tableName;
     ...
     * 获取数据表信息.
     *
     * @param mixed[]|string $tableName 数据表名 留空自动获取
    ...
    public function column(BaseQuery $query, $column, string $key = ''): array
    {
        ...
        if (empty($resultSet)) {
            ...
            if (strpos($column, ' ') !== false) {
                $column = (string) substr(strrchr(trim($column), ' '), 1);
            }
