...
trait WhereQuery
{
    ...
    public function where($field, $op = null, $condition = null)
    {
        ...
        if (is_array($field) && !empty($field) && $arrayIsListFunction($field)) {
        ...
        if ($via) {
            foreach ($this->options['where'] as $logic => &$where) {
                foreach ($where as $key => &$val) {
                    if (is_array($val) && strpos($val[0], '.') === false) {
                        $val[0] = $via . '.' . $val[0];
                    }
                }
            }
    ...
    protected function parseWhereExp(string $logic, $field, $op, $condition, array $param = [], bool $strict = false): self
    {
        ...
        // 处理 via
        if (is_string($field) && !empty($this->options['via']) && strpos($field, '.') === false && strpos($field, '->') === false) {
     ...
     */
    protected function executeQuery($query, $condition): void
    {
