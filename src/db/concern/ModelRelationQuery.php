...
trait ModelRelationQuery
{
     ...
     * @return $this
     */
    public function withSearch($fields, $data = [], bool $strict = false)
    {
        if (is_string($fields)) {
    ...
    public function withJoin($with, string $joinType = '')
    {
        ...
        foreach ($with as $key => $relation) {
            ...
            if ($relation instanceof Closure) {
                ...
                $field    = $relation;
                $relation = $key;
            } elseif (is_string($relation) && strpos($relation, '.') !== false) {
     ...
     * @return $this
     */
    protected function withAggregate($relations, string $aggregate = 'count', $field = 'id', bool $subQuery = true)
    {
        if (empty($this->model)) {
            return $this;
        }
     ...
     * @return $this
     */
    public function withCount($relation, string $field = 'id', bool $subQuery = true)
    {
     ...
     * @return $this
     */
    public function hasWhere($relation, $where = [], string $fields = '*', string $joinType = '')
    {
     ...
     * @return $this
     */
    public function hasWhereOr($relation, $where = [], string $fields = '*', string $joinType = '')
    {
