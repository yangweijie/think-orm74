...
abstract class BaseQuery
{
    ...
    public function __call(string $method, array $args)
    {
        if (strtolower(substr($method, 0, 5)) == 'getby') {
            // 根据某个字段获取记录
            $field = Str::snake((string) substr($method, 5));
        ...
        if (strtolower(substr($method, 0, 10)) == 'getfieldby') {
            // 根据某个字段获取记录的某个值
            $name = Str::snake((string) substr($method, 10));

            return $this->where($name, '=', $args[0])->value($args[1]);
        }

        if (strtolower(substr($method, 0, 7)) == 'whereor') {
            $name = Str::snake((string) substr($method, 7));
        ...
        if (strtolower(substr($method, 0, 5)) == 'where') {
            $name = Str::snake((string) substr($method, 5));
            array_unshift($args, $name);
     ...
     * @return array
     */
    public function column($field, string $key = '', bool $useModelAttr = false): array
    {
     ...
     * @return array
     */
    public function columnWithAttr($field, string $key = '')
    {
     ...
     * 查询SQL组装 union all.
     *
     * @param string|mixed[]|\Closure $union UNION数据
     *
     * @return $this
     */
    ...
    {
        if (!empty($this->options['via']) && strpos($field, '.') === false && strpos($field, '->') === false) {
     ...
     * @return bool
     */
    public function chunk(int $size, callable $callback, ?string $column = null, string $order = 'asc'): bool
    {
