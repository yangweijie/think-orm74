        ...
        return $this->filter(function ($item) use ($field, $operator, $value) {
            ...
            switch ($operator) {
                ...
                case '!==':
                    return $result !== $value;
                case 'in':
                    return is_scalar($result) && is_array($value) && in_array($result, $value);
                case 'not in':
                    return is_scalar($result) && is_array($value) && !in_array($result, $value);
                case 'between':
                    return is_array($value) && count($value) === 2 && ($result >= $value[0] && $result <= $value[1]);
                case 'not between':
                    return is_array($value) && count($value) === 2 && ($result < $value[0] || $result > $value[1]);
                case 'like':
                ...
                case 'not like':
                    return is_string($result) && strpos($result, $value) === false;
                case 'ilike':
                    return is_string($result) && strpos(strtolower($result), strtolower($value)) !== false;
                case 'start':
                    return is_string($result) && strncmp($result, $value, strlen($value)) === 0;
                case 'end':
                    return is_string($result) && substr_compare($result, $value, -strlen($value)) === 0;
                default:
                    return $result == $value;
            }
        });
